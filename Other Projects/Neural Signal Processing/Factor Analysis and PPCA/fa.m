...
hold on;
len = 12; 
plot([len*u(1,1)+mean(Xsim(:,1)) -len*u(1,1)+mean(Xsim(:,1))],[len*u(2,1)+mean(Xsim(:,2)) -len*u(2,1)+mean(Xsim(:,2))])
...
sig2 = 1/(8)*diag(diag(x_nor'*x_nor - ww*sum(x_nor.*[zmu' zmu']))) ;
lli(i)=sum(log(mvnpdf(Xsim,mu,(ww*ww'+sig2)))) ;
end
% ww=ww/norm(ww);
 plot(lli)
  xlabel('Iterations')
